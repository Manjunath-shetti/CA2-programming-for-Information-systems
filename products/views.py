...
def productlisting(request):
    cursor = connection.cursor()
    cursor.execute(
        "SELECT * FROM products_product, restaurant, type WHERE restaurant_id = product_restaurant_id AND type_id = product_type_id")
    ...
    # Message according Food Item #
    context['heading'] = "Food Items Details";
...
def cancel_order(request, orderID):
    ...
    cursor.execute("""
                UPDATE `order`
                SET order_status= '10' WHERE order_id = %s
            """, (
        orderID
    ))
...
def order_items(request, orderID):
    ...
    cursor.execute("SELECT *  FROM `products_product`, `order`, order_item, restaurant, type WHERE product_id =  oi_product_id AND oi_order_id = order_id AND restaurant_id = product_restaurant_id AND type_id = product_type_id AND order_id = "+ str(orderID))
    productlist = dictfetchall(cursor)
    ...
    ### Get the Total Cart  ####
    cursor.execute("SELECT SUM(oi_total) as totalCartCost  FROM `products_product`, `order`, order_item, restaurant, type WHERE product_id =  oi_product_id AND oi_order_id = order_id AND restaurant_id = product_restaurant_id AND type_id = product_type_id AND order_id = "+ str(orderID))
...
def product_filter(request, typeID):
    cursor = connection.cursor()
    cursor.execute(
        "SELECT * FROM products_product, restaurant, type WHERE restaurant_id = product_restaurant_id AND type_id = product_type_id AND type_id = "+ str(typeID))
    productlist = dictfetchall(cursor)
...
def update(request, productId):
    ...
    context = {
        "fn": "add",
        "prorestaurantlist":getDropDown('restaurant', 'restaurant_id', 'restaurant_name', productdetails.product_restaurant_id, '1'),
    ...
    if (request.method == "POST"):
        try:
            ...
            product_restaurant_id = request.POST['product_restaurant_id'],
        ...
        messages.add_message(request, messages.INFO, "Food Item updated succesfully !!!")
...
def product_details(request, productId):
    ...
    cursor = connection.cursor()
    cursor.execute(
        "SELECT * FROM products_product, restaurant, type WHERE restaurant_id = product_restaurant_id AND type_id = product_type_id AND product_id = "+productId)
...
def add(request):
    context = {
        "fn": "add",
        "prorestaurantlist":getDropDown('restaurant', 'restaurant_id', 'restaurant_name',0, '1'),
        "protypelist":getDropDown('type', 'type_id', 'type_name',0, '1'),
        "heading": 'Food Item add'
    };
    if (request.method == "POST"):
        try:
            product_image = None
            ...
            addFoodItem = product(product_name = request.POST['product_name'],
            product_type_id = request.POST['product_type_id'],
            product_restaurant_id = request.POST['product_restaurant_id'],
            product_price = request.POST['product_price'],
            product_image = product_image,                  
...
def delete(request, prodId):
    try:
        deleteFoodItem = product.objects.get(product_id = prodId)
        deleteFoodItem.delete()
    except Exception as e:
        return HttpResponse('Something went wrong. Error Message : '+ str(e))
    messages.add_message(request, messages.INFO, "Food Item Deleted Successfully !!!")
    return redirect('productlisting')
...
def stock(request):
    ...
    # Message according Food Item #
    context['heading'] = "Food Items Stock Details";
...
def restaurantlisting(request):
    ...
    # Message according Food Item #
    context['heading'] = "Food Items Restaurant";
    return render(request, 'viewrestaurant.html', context)
...
def order(request):
    ...
    context['heading'] = "Food Items Order Details";
